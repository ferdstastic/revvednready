...
function findDate(){
    ...
    if(currentTime.getMonth() == 11 && currentTime.getDate() > 15){
        christmasYear += 1; 
    }

    let christmasTime = new Date(christmasYear, 11, 15); 
    let dateDiff = Math.floor(christmasTime - currentTime);

    let DAYS = 0, HOURS = 0, MINUTES = 0, SECONDS = 0;

    if(currentTime.getMonth() != 11 || (currentTime.getMonth() == 11 && currentTime.getDate() != 15)){
